...
| ercot_demand    | ercot_demand_hourly.csv       | hourly  | electricity/rto/region-data (ERCO)             |
...
Usage
-----
  python update_eia_data.py                      # default recent window per dataset
  python update_eia_data.py --month              # from the 1st of this month
  python update_eia_data.py --since 2019-01-01   # backfill history
  python update_eia_data.py --only gas_price,generation_mix

Or import: from update_eia_data import update_all_eia_data
...
--------------------------------------------------------------------------
  ...
  ercot_*         3 days    - ~1 hr lag for demand, ~1 day for fuel mix
...
def make_session() -> requests_cache.CachedSession:
    session = requests_cache.CachedSession(
        ".cache",                      # -> .cache.sqlite, already gitignored
        expire_after=3600,
...
def fetch_ercot_demand(session, start: date, end: date) -> pd.DataFrame:
    rows = fetch_all(session, "electricity/rto/region-data", _hourly_params(start, end))
    if not rows:
        return pd.DataFrame(columns=["period", *REGION_TYPES.values()])
    df = pd.DataFrame(rows)
    df["period"] = pd.to_datetime(df["period"], format="%Y-%m-%dT%H")
    df["value"] = pd.to_numeric(df["value"], errors="coerce")
    out = df.pivot_table(index="period", columns="type", values="value", aggfunc="sum")
    return out.rename(columns=REGION_TYPES).reset_index()
...
DATASETS = [
    ...
    Dataset("ercot_demand", "ercot_demand_hourly.csv", fetch_ercot_demand, 3),
